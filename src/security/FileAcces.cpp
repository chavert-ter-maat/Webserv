...
std::string	FileAccess::swap_to_right_server_config(std::string uri, int port)
{
	...
	_limitClientBodySize = STANDARD__limitClientBodySize;
	if (!server->_limitClientBodySize.content_list.empty())
		_limitClientBodySize = std::stoi(server->_limitClientBodySize.content_list.back()); //check if there
	else
		_limitClientBodySize = STANDARD__limitClientBodySize;
...
ConfigContent	*FileAccess::find_location_config(std::string uri, ConfigContent *location_config, std::string method)
{
	...
	if (previous_match && (LocationStruct *)previous_match->childs && !((LocationStruct *)previous_match->childs)->_limitClientBodySize.content_list.empty())
		_limitClientBodySize = std::stoi(((LocationStruct *)previous_match->childs)->_limitClientBodySize.content_list.back());
