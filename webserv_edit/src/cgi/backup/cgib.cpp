...
std::string cgi::executeCGI(const std::string &path, const std::string &args, std::shared_ptr<Request> _request, std::string interpreter) {
  ...
  CgiParsing(_request->get_headers(), environ, _request, path, interpreter);
//   std::unordered_map<std::string, std::string> headers, char **environ, ServerStruct &serverinfo, std::shared_ptr<Request> _request, const std::string &path
  ...
  if (pipe(pipefd_out) == -1){// || pipe(pipefd_in)) {
    // throw "Pipe Failed";
  ...
  if (pid < 0) {
	...
	//add env variables: might have to be at the envpp
	if (interpreter.compare(""))
		argv_str.push_back(interpreter);
    argv_str.push_back(path);
    argv_str.push_back(args);
    for (const std::string &arg : argv_str) {
      argv.push_back(const_cast<char *>(arg.c_str()));
    }
	// argv_str.push_back("name=\"fileToUpload\"");
	// argv_str.push_back("filename=\"hoi.txt\"");
    argv.push_back(NULL);

	std::cerr << "LAUNCH\n";
	std::cerr << _request->get_body() << std::endl;
	// std::cout << argv[0] << std::endl;
	int i = 0;
	while (argv[++i])
		std::cerr << argv[i] << std::endl;
	std::cerr << "LAUNCH2\n";
    execve(argv[0], argv.data(), &envpp_new[0]);
	std::cout << argv[0] << std::endl;
